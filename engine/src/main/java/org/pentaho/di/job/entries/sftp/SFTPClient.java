...
public class SFTPClient {
  ...
  public void login( String password ) throws KettleJobException {
    ...
    int maxConnectionAttempts = 62; // setting up delays to 6 seconds max
    ...
    while ( true ) {
      try {
        if ( tryCreateNewConnection() ) {
          ...
          System.out.println( "Connection failed" ); //TODO DELETE ME
        }

        if ( --maxConnectionAttempts <= 0 ) {
          System.out.println( " JSchException max count met" ); //TODO DELETE ME
        ...
        System.out.println( " InterruptedException Caught " ); //TODO DELETE ME
  ...
  private boolean tryCreateNewConnection() throws KettleJobException {
    try {
      ...
      System.out.println( "Initial connection attempt failed: " + e.getMessage() ); //TODO CHANGE to err ME
      return false;
    }
  }
